...
public class OptionsBottomSheetDialogFragment extends BottomSheetDialogFragment {
	...
	public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		if (disableSortBy) {
			...
			sortByRow.setOnClickListener(view16 -> {
                if (listener != null) {
		...
		mainView.findViewById(R.id.show_direction_row).setOnClickListener(view15 -> {
            if (listener != null) {
                listener.showDirectionOnClick();
            }
            dismiss();
        });
		mainView.findViewById(R.id.coordinate_input_row).setOnClickListener(view14 -> {
            if (listener != null) {
                listener.coordinateInputOnClick();
            }
            dismiss();
        });
		mainView.findViewById(R.id.build_route_row).setOnClickListener(view13 -> {
            if (listener != null) {
		...
		if (disableSaveAsTrack) {
			...
			saveAsTrackRow.setOnClickListener(view12 -> {
                if (listener != null) {
		...
		if (disableMoveAllToHistory) {
			...
			moveAllToHistoryRow.setOnClickListener(view1 -> {
                if (listener != null) {
